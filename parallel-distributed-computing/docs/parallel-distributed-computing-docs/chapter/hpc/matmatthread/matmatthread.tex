\begin{chapter}{MatMatThread}
    \begin{section}{Problem at a glance}
        \par Given matrices $A$, $B$, $C$, $blockN1$, $blockN2$, $blockN3$, a number of $NTROW \times NTCOL$ threads, the goal is to compute the \glsxtrshort{gemm} operation $C = C + AB$ using \glsxtrshort{summa} in a multithreaded MatMatBlock approach.
    \end{section}
    \begin{section}{Algorithm}
        \begin{subsection}{Parallelization}
            \par The algorithm employs a multithreaded approach where each thread is responsible for executing a single \glsxtrshort{gemm} operation of a submatrix $C$.
